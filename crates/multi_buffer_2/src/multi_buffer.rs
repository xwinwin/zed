...
pub struct ExcerptId {
    pub initial_path: Option<Arc<Path>>,
...
impl ExcerptBoundary {
    pub fn starts_new_buffer(&self) -> bool {
        match (self.prev.as_ref(), self.next.as_ref()) {
            (None, _) => true,
            (Some(_), None) => false,
            (Some(prev), Some(next)) => prev.id.buffer_id != next.id.buffer_id,
        }
    }
}
...
impl MultiBuffer {
        ...
        T: Into<Arc<str>>,
    {
        ...
        for (ix, (range, new_text)) in edits.enumerate() {
            ...
            let buffer_start =
                start_excerpt.id.range.to_offset(&start_excerpt.buffer) + start_overshoot;
            edited_excerpt_ids.push(start_excerpt.id.clone());
            ...
            let buffer_end =
                .range
                end_excerpt.id.context.start.to_offset(&end_excerpt.buffer) + end_overshoot;
            ...
            if start_excerpt.id == end_excerpt.id {
                ...
                let start_excerpt_range =
                    buffer_start..start_excerpt.id.range.end.to_offset(&start_excerpt.buffer);
                let end_excerpt_range = end_excerpt
                    ...
                    ..buffer_end;
                buffer_edits
                    .entry(start_excerpt.id.buffer_id)
                    .or_insert(Vec::new())
                    .push(BufferEdit {
                ...
                buffer_edits
                    .entry(end_excerpt.id.buffer_id)
                    .or_insert(Vec::new())
                    .push(BufferEdit {
    ...
    pub fn insert_excerpts(&mut self, excerpts: impl IntoIterator<Item = (Model<Buffer>, Range<language::Anchor>)>) {}

    pub fn remove_excerpts(&mut self, excerpts: impl IntoIterator<Item = (Model<Buffer>, Range<language::Anchor>)>) {}
